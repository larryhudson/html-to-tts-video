...
const addSpansToText = (text, timings) => {
  ...
  timings.forEach((timing, index) => {
    ...
    const updatedSearchString = searchString.replace(
      timing.text,
      `<span data-timing-index="${index}">${timing.text}</span>`
    );
    workingString =
      workingString.substring(0, indexOfLastBracket + 1) + updatedSearchString;
  });
...
export const addSpansToHtml = (html, timingsArr) => {
  ...
  const treeWalker = document.createTreeWalker(
    document.body,
    4,
    (node) => !node.parentElement.classList.contains("hide-from-tts")
  );
  ...
  while (!timingEntry.done && treeWalker.nextNode()) {
    ...
    if (!hasMatch(treeWalker.currentNode.data, timingEntry.value[1].text))
      continue;

    // while the current node has the current timing text in it
    while (
      !timingEntry.done &&
      hasMatch(currentNode.data, timingEntry.value[1].text)
    ) {
